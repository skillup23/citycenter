...
function Footer() {
  return (
    <footer className="shrink-0 bg-white py-5 mt-16">
      <nav className="lg:container mx-auto flex justify-between items-center px-2 lg:pr-5">
        <Link
          href="/"
        ...
        <div className="grid grid-cols-4 md:flex items-center justify-between gap-2 sm:gap-4 md:gap-4 lg:gap-6 xl:gap-8 2xl:gap-12">
          {menu.slice(0, 8).map(({ id, name, link }) => {
            return (
              <Link
                key={id}
                href={link}
        ...
        <div className="hidden sm:flex ml-6">
          <Link href="https://vk.com/trkcitycentr" target={'_blank'}>
            <SlSocialVkontakte className="text-xl sm:text-2xl xl:text-4xl ease-in duration-200 hover:scale-105 text-black mr-5" />
          </Link>
          <Link href="https://t.me/ciiitycenter" target={'_blank'}>
            <FaTelegramPlane className="text-xl sm:text-4xl xl:text-4xl ease-in duration-200 hover:scale-105 text-black" />
          </Link>
        </div>
      </nav>
    </footer>
